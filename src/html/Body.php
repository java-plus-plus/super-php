...
class Body extends SuperPHPElement {
      ...
      * @param String|null onafterprint	Function to call after the user has printed the document.
      * @param String|null onbeforeprint	Function to call when the user requests printing of the document.
      * @param String|null onbeforeunload	Function to call when the document is about to be unloaded.
      * @param String|null onhashchange	Function to call when the fragment identifier part (starting with the hash (`'#'`) character) of the document's current address has changed.
      * @param String|null onlanguagechange	Function to call when the preferred languages changed.
      * @param String|null onmessage	Function to call when the document has received a message.
      * @param String|null onoffline	Function to call when network communication has failed.
      * @param String|null ononline	Function to call when network communication has been restored.
      * @param String|null onpagehide	undefined
      * @param String|null onpageshow	undefined
      * @param String|null onpopstate	Function to call when the user has navigated session history.
      * @param String|null onstorage	Function to call when the storage area has changed.
      * @param String|null onunload	Function to call when the document is going away.
      * @param String|null alink	undefined
      * @param String|null background	undefined
      * @param String|null bgcolor	undefined
      * @param String|null bottommargin	undefined
      * @param String|null leftmargin	undefined
      * @param String|null link	undefined
      * @param String|null onredo	undefined
      * @param String|null onundo	undefined
      * @param String|null rightmargin	undefined
      * @param String|null text	undefined
      * @param String|null topmargin	undefined
      * @param String|null vlink	undefined
      * 
      * * Global attributes:
     ...
     function __construct(
          ...
          // Element-specific attributes
          if ($onafterprint) $this->node->setAttribute("onafterprint", $onafterprint);
          if ($onbeforeprint) $this->node->setAttribute("onbeforeprint", $onbeforeprint);
          if ($onbeforeunload) $this->node->setAttribute("onbeforeunload", $onbeforeunload);
          if ($onhashchange) $this->node->setAttribute("onhashchange", $onhashchange);
          if ($onlanguagechange) $this->node->setAttribute("onlanguagechange", $onlanguagechange);
          if ($onmessage) $this->node->setAttribute("onmessage", $onmessage);
          if ($onoffline) $this->node->setAttribute("onoffline", $onoffline);
          if ($ononline) $this->node->setAttribute("ononline", $ononline);
          if ($onpagehide) $this->node->setAttribute("onpagehide", $onpagehide);
          if ($onpageshow) $this->node->setAttribute("onpageshow", $onpageshow);
          if ($onpopstate) $this->node->setAttribute("onpopstate", $onpopstate);
          if ($onstorage) $this->node->setAttribute("onstorage", $onstorage);
          if ($onunload) $this->node->setAttribute("onunload", $onunload);
          if ($alink) $this->node->setAttribute("alink", $alink);
          if ($background) $this->node->setAttribute("background", $background);
          if ($bgcolor) $this->node->setAttribute("bgcolor", $bgcolor);
          if ($bottommargin) $this->node->setAttribute("bottommargin", $bottommargin);
          if ($leftmargin) $this->node->setAttribute("leftmargin", $leftmargin);
          if ($link) $this->node->setAttribute("link", $link);
          if ($onredo) $this->node->setAttribute("onredo", $onredo);
          if ($onundo) $this->node->setAttribute("onundo", $onundo);
          if ($rightmargin) $this->node->setAttribute("rightmargin", $rightmargin);
          if ($text) $this->node->setAttribute("text", $text);
          if ($topmargin) $this->node->setAttribute("topmargin", $topmargin);
          if ($vlink) $this->node->setAttribute("vlink", $vlink);
